...
MessageConnection::MessageConnection(
    Network* owner_, NetworkServer* ownerServer_, Socket* socket_, ConnectionState startingState)
    : owner(owner_),
      ownerServer(ownerServer_),
      workerThread(0),
#ifdef KNET_THREAD_CHECKING_ENABLED
      workerThreadId(Thread::NullThreadId()),
#endif
      outboundAcceptQueue(16 * 1024),
      inboundMessageQueue(16 * 1024),
#ifdef KNET_NO_MAXHEAP
      outboundQueue(16 * 1024),
#endif
      inboundMessageHandler(0),
      socket(socket_),
...
bool MessageConnection::IsReadOpen() const
{
    ...
    if (socket && socket->IsOverlappedReceiveReady())  // If the socket is physically open, we are read-open.
        return true;
    if (!socket || connectionState == ConnectionPeerClosed ||
        connectionState == ConnectionClosed)  // Check against the main connectionState variable.
        return false;
    return true;
}
...
{
    return socket && socket->IsWriteOpen() && GetConnectionState() != ConnectionDisconnecting &&
           GetConnectionState() != ConnectionClosed;
}
...
void MessageConnection::Disconnect(int maxMSecsToWait)
{
    ...
    KNET_LOG(LogInfo,
        "MessageConnection::Disconnect(%d msecs): Write-closing connection. connectionState = %s, socket readOpen:%s, "
        "socket writeOpen:%s.",
...
void MessageConnection::Close(int maxMSecsToWait)  // [main thread]
{
    ...
    if (maxMSecsToWait > 0 && socket && socket->IsWriteOpen())
    {
        ...
        KNET_LOG(LogInfo,
            "MessageConnection::Close(%d msecs): Disconnecting. connectionState = %s, readOpen:%s, writeOpen:%s.",
            maxMSecsToWait, ConnectionStateToString(connectionState).c_str(),
            (socket && socket->IsReadOpen()) ? "true" : "false", (socket && socket->IsWriteOpen()) ? "true" : "false");
    }

    if (owner)
    {
        KNET_LOG(LogInfo, "MessageConnection::Close: Closed connection to %s.", ToString().c_str());
        owner->CloseConnection(this);  // This will cause this connection to be disconnected of its worker thread, so
                                       // that we can safely proceed to tear down the socket.
    ...
    if (socket)
    {
        socket->Close();
        assert(!IsWorkerThreadRunning());
        socket = 0;  // Worker thread assumes access to the socket pointer, so can't have the thread running any more
    ...
    if (outboundAcceptQueue.Size() > 0)
        KNET_LOG(LogVerbose, "MessageConnection::Close(): Had %d messages in outboundAcceptQueue!",
            (int)outboundAcceptQueue.Size());

    if (outboundQueue.Size() > 0)
        KNET_LOG(
            LogVerbose, "MessageConnection::Close(): Had %d messages in outboundQueue!", (int)outboundQueue.Size());

    if (inboundMessageQueue.Size() > 0)
        KNET_LOG(LogVerbose, "MessageConnection::Close(): Had %d messages in inboundMessageQueue!",
            (int)inboundMessageQueue.Size());

    if (fragmentedSends.UnsafeGetValue().transfers.size() > 0)
        KNET_LOG(LogVerbose, "MessageConnection::Close(): Had %d messages in fragmentedSends.transfers list!",
            (int)fragmentedSends.UnsafeGetValue().transfers.size());

    if (!fragmentedReceives.transfers.empty())
        KNET_LOG(LogVerbose, "MessageConnection::Close(): Had %d messages in fragmentedReceives.transfers list!",
            (int)fragmentedReceives.transfers.size());
...
void MessageConnection::SetPeerClosed()
{
    ...
    switch (connectionState)
    {
        ...
        default:
            KNET_LOG(LogError,
                "SetPeerClosed() called at an unexpected time. The internal connectionState has an invalid value %d!",
                (int)connectionState);
            assert(false);
            break;
    }
...
void MessageConnection::DetectConnectionTimeOut()
{
    ...
    {
        KNET_LOG(LogInfo,
            "It's been %.2fms since last heard from other end. connectionLostTimeout=%.2fms, so closing connection.",
...
void MessageConnection::AcceptOutboundMessages()  // [worker thread]
{
    ...
    if (connectionState == ConnectionDisconnecting || connectionState == ConnectionClosed)
    {
        while (outboundAcceptQueue.Size() > 0)
        {
            ...
            KNET_LOG(LogVerbose,
                "Warning: Discarding outbound network message with ID %d, since the connection is write-closed.",
                msg->id);
...
void MessageConnection::SplitAndQueueMessage(NetworkMessage* message, bool internalQueue, size_t maxFragmentSize)
{
    ...
    KNET_LOG(LogVerbose, "Splitting a message of %db into %d fragments of %db size at most.", (int)message->dataSize,
        (int)totalNumFragments, (int)maxFragmentSize);
    ...
    */
    ...
    {
        KNET_LOG(LogVerbose,
            "Upgraded a nonreliable message with ID %d and size %d to a reliable message since it had to be "
            "fragmented!",
            (int)message->id, (int)message->dataSize);
    }

    if (message->contentID != 0)
    {
        KNET_LOG(LogVerbose,
            "Warning: Content IDs are not supported with fragmented transfers. Removing the content ID %d of message "
            "%d of size %d.",
    ...
    while (byteOffset < message->dataSize)
    {
        ...
        fragment->reliable = true;  // We don't send fragmented messages as unreliable messages - the risk of a fragment
                                    // getting lost wastes bandwidth.
        fragment->messageNumber =
            outboundMessageNumberCounter++;  ///\todo Convert to atomic increment, or this is a race condition.
        ...
        fragment->reliableMessageNumber =
            outboundReliableMessageNumberCounter++;  ///\todo Convert to atomic increment, or this is a race condition.
#ifdef KNET_NETWORK_PROFILING
        ...
        if (internalQueue)  // if true, we are accessing from the worker thread, and can directly access the
                            // outboundQueue member.
        {
//            assert(ContainerUniqueAndNoNullElements(outboundQueue));
#ifdef KNET_NO_MAXHEAP
        ...
        else
        {
            if (!outboundAcceptQueue.Insert(fragment))
            {
                ...
                KNET_LOG(LogError, "Critical: Failed to add message fragment to outboundAcceptQueue! Queue was full. "
                                   "Do not know how to recover here!");
                assert(false);
            }
        }
    }
    ...
    // The original message that was split into fragments is no longer needed - it is represented by the newly created
    // fragments that have now been queued.
    FreeMessage(message);
}
...
void MessageConnection::EndAndQueueMessage(NetworkMessage* msg, size_t numBytes, bool internalQueue)
{
    ...
    {
        KNET_LOG(LogVerbose,
            "MessageConnection::EndAndQueueMessage: Discarded message with ID 0x%X and size %d bytes. "
            "msg->obsolete: %d. socket ptr: %p. ConnectionState: %s. socket->IsWriteOpen(): %s. msgconn->IsWriteOpen: "
            "%s. "
            "internalQueue: %s.",
            (int)msg->id, (int)numBytes, (int)msg->obsolete, socket,
            ConnectionStateToString(GetConnectionState()).c_str(), (socket && socket->IsWriteOpen()) ? "true" : "false",
    ...
    {
        KNET_LOG(LogError,
            "Critical! User specified a larger NetworkMessage than there is Capacity() for. Call "
            "NetworkMessage::Reserve() "
    ...
    ///\todo We can optimize here by doing the splitting at datagram creation time to create optimally sized datagrams,
    ///but
    /// it is quite more complicated, so left for later.
    const size_t sendHeaderUpperBound =
        32;  // Reserve some bytes for the packet and message headers. (an approximate upper bound)
    ...
    msg->reliableMessageNumber =
        (msg->reliable ? outboundReliableMessageNumberCounter++
                       : 0);  ///\todo Convert to atomic increment, or this is a race condition.
    msg->sendCount = 0;

    if (internalQueue)  // if true, we are accessing from the worker thread, and can directly access the outboundQueue
                        // member.
    {
        KNET_LOG(LogVerbose,
            "MessageConnection::EndAndQueueMessage: Internal-queued message of size %d bytes and ID 0x%X.",
            (int)msg->Size(), (int)msg->id);
//        assert(ContainerUniqueAndNoNullElements(outboundQueue));
#ifdef KNET_NO_MAXHEAP
        outboundQueue.InsertWithResize(msg);
#else
    ...
    else
    {
        ...
        {
            if (msg->reliable)  // For nonreliable messages it is not critical if we can't enqueue the message. Just
                                // discard it.
            {
                ///\todo Is it possible to check beforehand if this criteria is avoided, or if we are doomed?
                KNET_LOG(LogVerbose, "Critical: Failed to add new reliable message to outboundAcceptQueue! Queue was "
                                     "full. Discarding the message!");
                assert(false);
            }
            FreeMessage(msg);
            return;
        }
        KNET_LOG(LogData, "MessageConnection::EndAndQueueMessage: Queued message of size %d bytes and ID 0x%X.",
            (int)msg->Size(), (int)msg->id);
    }
...
void MessageConnection::Process(int maxMessagesToProcess)
{
    ...
    // to process, we will return immediately (won't wait for this many messages to actually be received, it is just an
    // upper limit).
    ...
    while (inboundMessageQueue.Size() > 0 && (numMessagesLeftToProcess-- > 0 || maxMessagesToProcess == 0))
    {
        ...
        {
            KNET_LOG(LogVerbose,
                "Warning! Cannot process messages since no message handler registered to connection %s!",
                ToString().c_str());
            return;
        }
        ...
        inboundMessageHandler->HandleMessage(
            this, msg->receivedPacketID, msg->id, (msg->dataSize > 0) ? msg->data : 0, msg->dataSize);
...
void MessageConnection::WaitForMessage(int maxMSecsToWait)  // [main thread]
{
    ...
    else
    {
        ...
        {
            KNET_LOG(LogWaits,
                "MessageConnection::WaitForMessage: Waited %f msecs for a new message. ConnectionState: %s. %d "
                "messages in queue.",
                timer.MSecsElapsed(), ConnectionStateToString(GetConnectionState()).c_str(),
                (int)inboundMessageQueue.Size());
        }
...
int NetworkMessage::GetTotalDatagramPackedSize() const
{
    ...
    //    const int fragmentStartLength = (transfer && fragmentIndex == 0) ?
    //    VLE8_16_32::GetEncodedBitLength(transfer->totalNumFragments)/8 : 0; const int fragmentLength = (transfer ? 1 :
    //    0) + ((transfer && fragmentIndex != 0) ? VLE8_16_32::GetEncodedBitLength(fragmentIndex)/8 : 0);
...
void MessageConnection::CheckAndSaveOutboundMessageWithContentID(NetworkMessage* msg)
{
    ...
    if (iter != outboundContentIDMessages
                    .end())  // We have a previous message in the queue which is now obsoleted by this message.
    {
        // Sanity check: The message numbers must be in the proper order. msg must have been admitted later to send
        // queue than the existing message.
        ...
        {
            KNET_LOG(LogError,
                "Warning! Adding new message ID %d, number %d, content ID %d, priority %d, but it was obsoleted by an "
                "already existing message number %d.",
                (int)msg->id, (int)msg->messageNumber, (int)msg->contentID, (int)iter->second->priority,
                (int)iter->second->messageNumber);
            msg->obsolete = true;
        }
    }
    else
...
bool MessageConnection::CheckAndSaveContentIDStamp(message_id_t messageID, u32 contentID, packet_id_t packetID)
{
    ...
    {
        if (PacketIDIsNewerThan(packetID, iter->second.first) ||
            (float)Clock::TimespanToMillisecondsD(iter->second.second, now) > 5.f * 1000.f)
        {
...
void MessageConnection::HandleInboundMessage(packet_id_t packetID, const char* data, size_t numBytes)
{
    ...
    {
        KNET_LOG(LogError, "Error parsing messageID of a message in socket %s. Data size: %d bytes.",
            socket->ToString().c_str(), (int)numBytes);
        throw NetException(
            "MessageConnection::HandleInboundMessage: Network error occurred when deserializing message ID VLE field!");
    }
    KNET_LOG(LogData, "Received message with ID %d and size %d from peer %s.", (int)packetID, (int)numBytes,
    ...
    switch (messageID)
    {
        ...
        default:
        {
            ...
            {
                KNET_LOG(LogError,
                    "Failed to add a new message of ID %d and size %dB to inbound queue! Queue was full.",
...
void MessageConnection::HandlePingRequestMessage(const char* data, size_t numBytes)
{
    ...
    {
        KNET_LOG(
            LogError, "Malformed PingRequest message received! Size was %d bytes, expected 1 byte!", (int)numBytes);
        return;
    }
...
void MessageConnection::HandlePingReplyMessage(const char* data, size_t numBytes)
{
    ...
    // How much to bias the new rtt value against the old rtt estimation. 1.f - 100% biased to the new value. near zero
    // - very stable and nonfluctuant.
    ...
    for (size_t i = 0; i < cs.ping.size(); ++i)
        if (cs.ping[i].pingID == pingID && cs.ping[i].replyReceived == false)
        {
            ...
            float newRtt = (float)Clock::TicksToMillisecondsD(
                Clock::TicksInBetween(cs.ping[i].pingReplyTick, cs.ping[i].pingSentTick));
    ...
    statistics.Unlock();
    KNET_LOG(LogError, "Received PingReply with ID %d in socket %s, but no matching PingRequest was ever sent!",
        (int)pingID, socket->ToString().c_str());
}
...
void MessageConnection::DumpStatus() const
{
    ...
    sprintf(str,
        ...
        ConnectionStateToString(GetConnectionState()).c_str(), (int)NumInboundMessagesPending(),
        (int)NumOutboundMessagesPending(), Connected() ? "connected" : "", IsReadOpen() ? "readOpen" : "",
        IsWriteOpen() ? "writeOpen" : "", socket ? "exists" : "zero",
        (socket && socket->Connected()) ? "connected" : "", (socket && socket->IsReadOpen()) ? "readOpen" : "",
        (socket && socket->IsWriteOpen()) ? "writeOpen" : "", RoundTripTime(), LastHeardTime(), PacketsInPerSec(),
        PacketsOutPerSec(), MsgsInPerSec(), MsgsOutPerSec(), FormatBytes(BytesInPerSec()).c_str(),
        FormatBytes(BytesOutPerSec()).c_str(), (int)eventMsgsOutAvailable.Test(),
#ifdef _WIN32
        ...
        -1,
#endif
        (socket && socket->GetOverlappedSendEvent().Test()) ? "true" : "false", (int)TimeUntilCanSendPacket(),
...
void MessageConnection::AssertInWorkerThreadContext() const
{
    ...
    {
        KNET_LOG(LogError,
            "Assert failure in MessageConnection::AssertInWorkerThreadContext()!: haveWorkerThread: %s, "
            "currentThreadId: %s, workerThreadId: %s,",
            haveWorkerThread ? "true" : "false", ThreadIdToString(currentThreadId).c_str(),
            ThreadIdToString(workerThreadId).c_str());
...
void MessageConnection::AssertInMainThreadContext() const
{
    ...
    {
        KNET_LOG(LogError,
            "Assert failure in MessageConnection::AssertInMainThreadContext()!: haveWorkerThread: %s, currentThreadId: "
            "%s, workerThreadId: %s,",
            haveWorkerThread ? "true" : "false", ThreadIdToString(currentThreadId).c_str(),
            ThreadIdToString(workerThreadId).c_str());
