...
#include "BasicSerializedDataTypes.hpp"
...
struct ConnectionStatistics
{
    ...
    struct PingTrack
    {
        ...
        tick_t pingReplyTick;  ///< If replyReceived==true, contains the timestamp of when PingReply was received as a
                               ///response.
...
public:
    int operator()(const NetworkMessage* a, const NetworkMessage* b)
    {
        ...
        if (a->MessageNumber() < b->MessageNumber())
            return 1;
        if (b->MessageNumber() < a->MessageNumber())
            return -1;
...
{
    ConnectionPending,        ///< Waiting for the other end to send an acknowledgement packet to form the connection. No
                              ///messages may yet be sent or received at this state.
    ConnectionOK,             ///< The connection is bidirectionally open, for both reading and writing. (readOpen=true,
                              ///writeOpen=true)
    ConnectionDisconnecting,  ///< We are closing the connection down. Cannot send any more messages, but can still
                              ///receive. (readOpen=true, writeOpen=false)
    ConnectionPeerClosed,     ///< The other end has closed the connection. No new messages will be received, but can still
                              ///send messages. (readOpen=false, writeOpen=true)
    ConnectionClosed          ///< The socket is no longer open. A MessageConnection object in this state cannot be reused to
                              ///open a new connection, but a new connection object must be created.
};
...
public:
    ...
    /// Returns true if the peer has signalled it will not send any more data (the connection is half-closed or
    /// full-closed).
    ...
    /// Returns true if the connection is in the ConnectionPending state and waiting for the other end to
    /// resolve/establish the connection. When this function returns false, the connection may be half-open,
    /// bidirectionally open, timed out on ConnectionPending, or closed.
    ...
    /// Runs a modal processing loop and produces events for all inbound received data. Returns when the connection is
    /// closed. This is an example function mostly useful only for very simple demo applications. In most cases, you do
    ...
    ///                 NetworkMessage::data field to hold at least that many bytes (Capacity() can also return a larger
    ///                 value). This number only needs to be an estimate, since you can later on call
    ///                 NetworkMessage::Reserve() to reallocate the message memory. If you pass in the default value 0,
    ///                 no pre-allocation will be performed.
    /// @return The NetworkMessage object that represents the new message to be built. This message is dynamically
    /// allocated
    ///         from an internal pool of NetworkMessage blocks. For each NetworkMessage pointer obtained, call
    ///         EndAndQueueMessage when you have finished building the message to commit the network send and to release
    ///         the memory. Alternatively, if after calling StartNewMessage, you decide to abort the network send, free
    ///         up the NetworkMessage by calling this->FreeMessage().
    ...
    /// @param msg The message to send. After calling this function, this pointer should be considered freed and may not
    /// be
    ///            dereferenced or passed to any other member function of this MessageConnection. Only pass in here
    ///            NetworkMessage pointers obtained by a call to StartNewMessage() of the same MessageConnection
    ///            instance.
    /// @param numBytes Specify here the number of actual bytes you filled in into the msg.data field. A size of 0
    ///                 is valid, and can be used in cases the message ID itself is the whole message. Passing in the
    ///                 default value of this parameter will use the size value that was specified in the call to
    ///                 StartNewMessage().
    /// @param internalQueue If true, specifies that this message was submitted from the network worker thread and not
    /// the application
    ///                 thread. Pass in the value 'false' here in the client application, or there is a chance of a race
    ///                 condition.
    void EndAndQueueMessage(
        NetworkMessage* msg, size_t numBytes = (size_t)(-1), bool internalQueue = false);  // [main and worker thread]

    /// This is a conveniency function to access the above StartNewMessage/EndAndQueueMessage pair. The performance of
    /// this function call is not as good, since a memcpy of the message will need to be made. For performance-critical
    /// messages, it is better to craft the message directly into the buffer area provided by StartNewMessage.
    ...
    void SendStruct(const SerializableData& data, unsigned long id, bool inOrder, bool reliable, unsigned long priority,
        unsigned long contentID = 0);  // [main thread]
    ...
    /// Stops all outbound sends until ResumeOutboundSends is called. Use if you need to guarantee that some messages be
    /// sent in the same datagram. Do not stop outbound sends for long periods, or the other end may time out the
    /// connection.
    ...
    /// Returns the number of messages that have been received from the network but haven't been handled by the
    /// application yet.
    ...
    {
        return outboundQueue.Size() + outboundAcceptQueue.Size();
    }  // [main and worker thread]

    /// Returns the number of outbound messages the main thread has queued for the worker thread to send out. (still
    /// unaccepted by the worker thread).
    size_t OutboundAcceptQueueSize() const { return outboundAcceptQueue.Size(); }  // [main and worker thread]

    /// Returns the number of outbound messages in the worker thread outbound message queue (already accepted and
    /// pending a send by the worker thread).
    ...
    /// @param numBytesPerSec The upper limit for the number of bytes to send per second. This limit includes the
    /// message header
    ///                       bytes as well and not just the payload. Set to 0 to force no limit.
    /// @param numDatagramsPerSec The maximum number of datagrams (UDP packets) to send per second. Set to 0 to force no
    /// limit.
    ...
    ///                            the processing loop will return to give processing time to other parts of the
    ///                            application. If 0 is passed, messages are processed until the queue is empty.
    /// \note It is important to have a non-zero limit in maxMessagesToProcess (unless you're sure what you are doing),
    /// since
    ///       otherwise an attacker might affect the performance of the application main loop by sending messages so
    ///       fast that the queue never has time to exhaust, thus giving an infinite loop in practice.
    ...
    /// @param maxMSecsToWait If 0, the call will wait indefinitely until a message is received or the connection
    /// transitions to
    ///                       closing state.
    ///                       If a positive value is passed, at most that many milliseconds is waited for a new message
    ///                       to be received.
    void WaitForMessage(int maxMSecsToWait);  // [main thread]

    /// Returns the next message in the inbound queue. This is an alternative API to
    /// RegisterInboundMessageHandler/Process. \note When using this function to receive messages, remember to call
    /// FreeMessage for each NetworkMessage returned, or you will have a major size memory leak, fast.
    /// @param maxMSecsToWait If a negative number, the call will not wait at all if there are no new messages to
    /// process, but
    ///                       returns 0 immediately.
    ///                       If 0, the call will wait indefinitely until a message is received or the connection
    ///                       transitions to closing state. If a positive value is passed, at most that many
    ///                       milliseconds is waited for a new message to be received.
    /// @return A newly allocated object to the received message, or 0 if the queue was empty and no messages were
    /// received during
    ///         the wait period, or if the connection transitioned to closing state. When you are finished reading the
    ///         message, call FreeMessage for the returned pointer.
    ...
    /// Returns the estimated RTT of the connection, in milliseconds. RTT is the time taken to communicate a message
    /// from client->host->client.
    ...
    {
        return Clock::TicksToMillisecondsF(Clock::TicksInBetween(Clock::Tick(), lastHeardTime));
    }  // [main and worker thread]
    ...
    /// Returns the total number of bytes (excluding IP and TCP/UDP headers) that have been received from this
    /// connection.
...
protected:
    ...
    /// Performs a check that asserts that the current thread of execution is not in the network worker thread (it is
    /// the main thread).
    ...
    /// A queue populated by the networking thread to hold all the incoming messages until the application can process
    /// them.
...
///\todo Make the choice of which of the following structures to use a runtime option.
#ifndef KNET_NO_MAXHEAP                                                 // If defined, disables message priorization feature to improve client-side CPU performance. By
                                                                        // default disabled.
    ...
    /// Allocations of NetworkMessage structures go through a pool to avoid dynamic new/delete calls when sending
    /// messages. This structure is shared between the main and worker thread through a lockfree construct.
    ...
    void AddOutboundStats(
        unsigned long numBytes, unsigned long numPackets, unsigned long numMessages);  // [worker thread]

    /// Adds a new entry for inbound data statistics.
    void AddInboundStats(
        unsigned long numBytes, unsigned long numPackets, unsigned long numMessages);  // [worker thread]

    /// Pulls in all new messages from the main thread to the worker thread side and admits them to the send priority
    /// queue.
    ...
    Socket* socket;  // [set by main thread before the worker thread is running. Read-only when worker thread is
                     // running. Read by main and worker thread]
    ...
    float bytesInPerSec;     ///< The average number of bytes we are receiving/second. This includes kNet headers. [main
                             ///and worker thread]
    float bytesOutPerSec;    ///< The average number of bytes we are sending/second. This includes kNet headers. [main and
                             ///worker thread]
    u64 bytesInTotal;
    u64 bytesOutTotal;
    ...
    void SplitAndQueueMessage(
        NetworkMessage* message, bool internalQueue, size_t maxFragmentSize);  // [main and worker thread]
...
template <typename SerializableData>
void MessageConnection::SendStruct(const SerializableData& data, unsigned long id, bool inOrder, bool reliable,
    unsigned long priority, unsigned long contentID)
{
