// The UDPMessageConnection class.
//
...
#include "SequentialIntegerSet.hpp"
...
1bit - FRAGMENT. This message is a fragment of a bigger message -flag. If FRGSTART=true, this field is not read and is
assumed to be true. 1bit - InOrder. This message may be processed by the application only after all the previous InOrder
messages have been processed. * 1bit - InOrder. This message may be processed by the application only after all the
previous InOrder messages have been processed. * 1bit - InOrder. This message may be processed by the application only
after all the previous InOrder messages have been processed. (Old: 1bit - Unused.) (Old: 1bit - Unused.) 11 bits Content
length (includes both the length of MessageID and Content Data fields) * u8 InOrder array index.
Only present if InOrder=7 (111 in base 2). u8-u32 # of Fragments in whole message  VLE-encoded 1.7/1.7/16    Only
present if FRGSTART=true. u8 Fragment Transfer ID                                           Only present if
FRAGMENT=true or FRGSTART=true. * New: u8-u32 Fragment number           VLE-encoded 1.7/1.7/16    Only present if
FRAGMENT=true and FRGSTART=false. (Old: u8-u16 Fragment number            VLE-encoded 1.7/8         Only present if
FRAGMENT=true.) .Content.
...
private:
    ...
    bool HandleMessage(
        packet_id_t packetID, message_id_t messageID, const char* data, size_t numBytes);  // [worker thread]
    ...
    /// Copies the given message to an internal queue to wait to be processed by the worker thread that owns this
    /// connection.
    ...
    /// Specifies in milliseconds the currently recommended datagram timeout value for any packet that is sent out at
    /// the present time.
    ...
    {
        PacketAckTrack() : sendCount(0) {}

        /// The timestamp of when a packet with this ID was last sent out.
        tick_t sentTick;

        /// The timestamp of when this packet will time out and can be resent (at the earliest. It will get queued and
        /// be resent later)
        tick_t timeoutTick;
        ...
        /// The number of times this packet has been sent. 1 denotes no resends. 2 - this packet's been resent once, and
        /// so on.
        int sendCount;
    ...
    static int BiasedBinarySearchFindPacketIndex(
        UDPMessageConnection::PacketAckTrackQueue& queue, packet_id_t packetID);
