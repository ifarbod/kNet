...
public:
    ...
    /// Instantiates a new DataSerializer that maintains its own buffer space for serialization and serializes using a
    /// template.
    ///\todo Support dynamic resizing.
    ...
    /// Instantiates a new DataSerializer that writes its data into the given fixed-size buffer, using a message
    /// template.
    ...
    /// aid in serialization, the type T may be any of the types bit, u8, s8, u16, s16, u32, s32, u64, s64, float,
    /// double, const char * or std::string. If you are not using a serialization template, you may pass in any type
    /// that is a POD type and can be reinterpret_casted to a u8 buffer and memcpy'd to a byte buffer.
    template <typename T>
    ...
    /// @param value The variable where the bits are taken from. The bits are read from the LSB first, towards the MSB
    /// end of the value.
    ...
    /// @param value The floating-point value to send. This float must have a value in the range [-2^(numIntegerBits-1),
    /// 2^(numIntegerBits-1)[.
    /// @param numIntegerBits The number of bits to use to represent the integer part.
    /// @param numDecimalBits The number of bits to use to represent the fractional part.
    /// @note Before writing the value, it is clamped to range specified above to ensure that the written value does not
    ...
    /// Writes the given float quantized to the number of bits, that are distributed evenly over the range [minRange,
    /// maxRange].
    /// @param value The floating-point value to send. This float must have a value in the range [minRange, maxRange].
    /// @param numBits The number of bits to use for representing the value. The total number of different values
    /// written is then 2^numBits,
    ///     which are evenly distributed across the range [minRange, maxRange]. The value numBits must satisfy 1 <=
    ///     numBits <= 30.
    ...
    /// @param exponentBias For IEEE-754 floats, the signed exponent is converted to unsigned number by adding an offset
    /// bias.
    ///                This field specifies the bias to use. Usually it is ok to reserve the equal number of exponent
    ///                values for negative and positive exponents, meaning that exponentBias == (1 << (exponentBits -
    ///                1)) - 1 is an ok default.
    ...
    /// Writes the given normalized 2D vector compressed to a single 1D polar angle value. Then the angle is quantized
    /// to the specified precision.
    /// @param x The x coordinate of the 2D vector.
    /// @param y The y coordinate of the 2D vector.
    /// @param numBits The number of bits to quantize the representation down to. This value must satisfy 1 <= numBits
    /// <= 30.
    /// @note The vector (x,y) does not need to be normalized for this function to work properly (don't bother enforcing
    /// normality in
    ...
    /// @important This function does not write a fixed amount of bits to the stream, but omits the direction if the
    /// length is zero.
    ...
    /// Writes the given normalized 3D vector converted to spherical form (azimuth/yaw, inclination/pitch) and quantized
    /// to the specified range. The given vector (x,y,z) must be normalized in advance.
    ...
    /// @note After converting the euclidean (x,y,z) to spherical (yaw, pitch) format, the yaw value is expressed in the
    /// range [-pi, pi] and pitch
    ///     is expressed in the range [-pi/2, pi/2]. Therefore, to maintain consistent precision, the condition
    ///     numBitsYaw == numBitsPitch + 1 should hold. E.g. If you specify 8 bits for numBitsPitch, then you should
    ///     specify 9 bits for numBitsYaw to have yaw & pitch use the same amount of precision.
    /// @note This function uses the convention that the +Y axis points towards up, i.e. +Y is the "Zenith direction",
    /// and the X-Z plane is the horizontal
    ///     "map" plane.
    void AddNormalizedVector3D(float x, float y, float z, int numBitsYaw, int numBitsPitch);

    /// Writes the given 3D vector converted to spherical form (azimuth/yaw, inclination/pitch, length) and quantized to
    /// the specified range.
    /// @param numBitsYaw The number of bits to use for storing the azimuth/yaw part of the vector.
    /// @param numBitsPitch The number of bits to use for storing the inclination/pitch part of the vector.
    /// @param magnitudeIntegerBits The number of bits to use for the integral part of the vector's length. This means
    ...
    /// and the X-Z plane is the horizontal
    ///     "map" plane.
    int AddVector3D(float x, float y, float z, int numBitsYaw, int numBitsPitch, int magnitudeIntegerBits,
        int magnitudeDecimalBits);
    ...
    void AddArithmeticEncoded(
        int numBits, int val1, int max1, int val2, int max2, int val3, int max3, int val4, int max4);
    void AddArithmeticEncoded(int numBits, int val1, int max1, int val2, int max2, int val3, int max3, int val4,
    ...
    /// Returns the total number of bits that can still be serialized into this DataSerializer object before overflowing
    /// (which throws an exception).
    size_t BitsLeft() const { return Capacity() * 8 - BitsFilled(); }

    /// Returns the total number of full bytes that can still be serialized into this DataSerializer object before
    /// overflowing (which throws an exception).
...
public:
    ...
    static void DeserializeFrom(DataDeserializer& src, T& dst) { dst.DeserializeFrom(src); }
};
...
{
public:
    static size_t Size(const std::string& value) { return value.length() + 1; }
    ...
    static void DeserializeFrom(DataDeserializer& src, std::string& dst) { dst = src.ReadString(); }
};
}  // ~kNet
