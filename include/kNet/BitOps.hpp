...
public:
    ...
    enum
    {
        val = LSBT<Pos + Bits>::val & ~LSBT<Pos>::val
    };  // But this is nicer.
};
...
template <typename ResultType, typename InputType, int APos, int ABits, int RPos, int RBits, int GPos, int GBits,
    int BPos, int BBits>
ResultType PackBits(InputType a, InputType r, InputType g, InputType b)
{
    return (ResultType)((ResultType)((a << APos) & BitMaskT<APos, ABits>::val) |
...
ResultType PackBits(int APos, int ABits, int RPos, int RBits, int GPos, int GBits, int BPos, int BBits, InputType a,
    InputType r, InputType g, InputType b)
{
    return (ResultType)(
        (ResultType)((a << APos) & BitMask(APos, ABits)) | (ResultType)((r << RPos) & BitMask(RPos, RBits)) |
        (ResultType)((g << GPos) & BitMask(GPos, GBits)) | (ResultType)((b << BPos) & BitMask(BPos, BBits)));
}
