...
//   Provides global memory allocation routines using instance-based allocators.
...
//{
// public:
//    static void *Alloc(Allocator *instance, size_t size, size_t alignment, const char *nameTag = 0, AllocFlags flags
//= AFAllocLow);     static void Free(Allocator *instance, void *ptr);     static size_t Size(Allocator *instance,
// void *ptr);
//};

// The reason that the member functions are static, but still take in as the first parameter an instance of the
// Allocator  is that this allows both class-based and instance-based allocators to be created using the same interface.
...
{
    AFAllocLow =
        0x1,  ///< Allocate from the low end of the memory space (if applicable for the given Allocator). Default.
...
template <typename Object, typename Allocator>
inline Object* NewArray(
    size_t numObjects, Allocator* allocator, size_t alignment, const char* nameTag = 0, AllocFlags flags = AFAllocLow)
{
